import { Circle, Layout, Line, Ray, contrast, makeScene2D } from '@motion-canvas/2d';
import { Logger, PossibleVector2, SignalValue, SimpleSignal, Vector2, Vector2Signal, all, arcLerp, chain, clamp, createEffect, createRef, createSignal, delay, linear, loop, makeRef, range, sequence, sin, tween, useLogger, waitFor } from '@motion-canvas/core';
...
function dipFieldAtPoint(p: Vector2, qPos: Vector2, qNeg: Vector2): Vector2 {
  ...
  const theta = Vector2.angleBetween(diff, p);
...
export default makeScene2D(function* (view) {
  ...
  view.add(<Layout ref={container}></Layout>);
  ...
  range(numLines).map(idx => {
    ...
    while (currChargePoints.length < 1000 && lastPoint().sub(chargeNeg().position()).magnitude > 50) {
      ...
      if (lastPoint().x > view.width() / 2) {
        outsideBounds.push(createSignal(new Vector2(-view.width() / 2, lastPoint().y)));
        break;
      }
      if (lastPoint().y > (view.height() / 2)) {
        outsideBounds.push(createSignal(new Vector2(-lastPoint().x, view.height() / 2)));
        break;
      }
      if (lastPoint().y < -(view.height() / 2)) {
        outsideBounds.push(createSignal(new Vector2(-lastPoint().x, -view.height() / 2)));
